...
class BaseMentaLLaMA(MentaLLaMAInterface):
    """
    Abstract base class for all MentaLLaMA service implementations.
    
    Defines the common interface and base functionality for all
    MentaLLaMA language model implementations.
    """
    
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        """
        Initialize the base MentaLLaMA service.
    ...
    def initialize(self, config: Dict[str, Any]) -> None:
        ...
        logger.info("Base MentaLLaMA service initialized")
    ...
    def shutdown(self) -> None:
        ...
        self._models.clear()
        self._config.clear()
        logger.info("Base MentaLLaMA service shut down")
...
class MentaLLaMA(BaseMentaLLaMA):
    """
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self._phi_detection_service = None
    ...
    def initialize(self, config: Dict[str, Any]) -> None:
        ...
        Raises:
            InvalidConfigurationError: If configuration is invalid
        """
        # Extract PHI detection service if provided
        if "phi_detection_service" in config:
            self._phi_detection_service = config.pop("phi_detection_service")
            logger.info("PHI detection service attached to MentaLLaMA")
        
        # Initialize base implementation
        super().initialize(config)
        
        # Initialize NLP models and connections
        sanitized_config = {k: "***" if "key" in k.lower() or "secret" in k.lower() else v for k, v in self._config.items()}
        logger.info("Initializing MentaLLaMA service with config: %s", json.dumps(sanitized_config))
