...
class RolePermission:
    ...
    middleware.rbac.has_permission.return_value = True # Default to allow for basic tests
    return middleware


    # Custom Mock Headers class to handle case-insensitivity and MagicMock interaction
    class MockHeaders:
    ...
    mock_request.scope = {"type": "http", "method": "GET", "path": "/api/patients", "headers": []} # Basic scope
    ...
    async def test_valid_authentication(self, auth_middleware, authenticated_request, mock_jwt_service):
        ...
        # Setup the next middleware in the chain
        async def mock_call_next(request: Request) -> Response:
        pytest.fail("Middleware should have blocked the request") # pragma: no cover
        return JSONResponse(content={"status": "success"}) # pragma: no cover
        ...
        mock_jwt_service.validate_token.return_value = MagicMock()
        is_valid=True, status="VALID",
        claims={"sub": "user123", "name": "Nurse Johnson", "roles": ["nurse"], "scopes": []},
        token_type="access", error=None
        (    )
        ...
        def rbac_side_effect(roles, permission):
            if "nurse" in roles and permission == "read:patients":
                ...
                async def mock_call_next_success(request: Request) -> Response:
        #     return JSONResponse(content={"status": "success"}) # FIXME: return outside function
        ...
        async def mock_call_next_fail(request: Request) -> Response:
        pytest.fail("Middleware should have blocked the request") # pragma: no cover
        #     return JSONResponse(content={"status": "success"}) # pragma: no cover # FIXME: return outside function
        ...
        mock_jwt_service.validate_token.return_value = MagicMock()
        is_valid=True, status="VALID",
        claims={"sub": "admin123", "name": "Admin User", "roles": ["admin"], "scopes": []},
        token_type="access", error=None
        (    )
        ...
        mock_jwt_service.validate_token.return_value = MagicMock()
        is_valid=True, status="VALID",
        claims={"sub": "user123", "roles": ["patient"], "patient_id": "PT12345"},
        token_type="access", error=None
        (    )
        # Mock RBAC: allow if permission is read:own_records and patient_id matches
        def rbac_side_effect(roles, permission, resource_id=None, user_id=None):
             ...
             if permission == "read:own_records" and resource_id == "PT12345":
                 ...
                 authenticated_request.scope["path_params"] = {"patient_id": patient_id_in_path}
        ...
        authenticated_request.scope["path_params"] = {"patient_id": other_patient_id}
        ...
        @pytest.mark.asyncio
        async def test_audit_logging(self, auth_middleware, authenticated_request, unauthenticated_request):
        ...
        # Add more tests for edge cases, different auth backends (Cognito), etc.