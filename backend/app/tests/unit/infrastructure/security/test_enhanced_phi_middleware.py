...
def test_app(mock_audit_logger):
    ...
    @app.get("/test")
async def test_endpoint():    return {"message": "Test endpoint"}
    
        @app.get("/test-with-phi")
async def test_with_phi():    return {"patient": "John Doe", "ssn": "123-45-6789"}
    
        @app.post("/test-post")
async def test_post(data: dict):    return data
...
class TestEnhancedPHIMiddleware:
    ...
    def test_non_json_response_not_sanitized(self, test_app, test_client):
        ...
        @test_app.get("/test-text")
                                        async def test_text():    return "This is a text response with John Doe's information"
    ...
    def test_error_handling_in_response_processing(self, mock_logger, test_app, test_client):
        ...
        @test_app.get("/test-error")
            ...
            # Return a response that will cause a JSON decode error    return JSONResponse(content=b"invalid json")
