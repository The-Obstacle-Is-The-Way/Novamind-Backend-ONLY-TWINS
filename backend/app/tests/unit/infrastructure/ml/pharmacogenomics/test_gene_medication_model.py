...
class TestGeneMedicationModel:
    ...
    async def test_lookup_known_interactions(self, model):
        ...
        fluoxetine_interaction = next((i for i in interactions if i["medication"] == "fluoxetine"), None)
    ...
    async def test_predict_novel_interactions(self, model):
        ...
        model._model.predict = MagicMock(return_value=np.array([1, 0, 0]))  # Interaction with fluoxetine only
