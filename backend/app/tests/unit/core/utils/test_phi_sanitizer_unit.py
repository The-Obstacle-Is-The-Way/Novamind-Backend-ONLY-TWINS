...
class TestPHISecureLogger:
    ...
    def phi_secure_logger(self):
        ...
        logger.info = lambda msg, * \
            args, **kwargs: original_info(PHISanitizer.sanitize_string(msg), *args, **kwargs)
        logger.warning = lambda msg, * \
            args, **kwargs: original_warning(PHISanitizer.sanitize_string(msg), *args, **kwargs)
        logger.error = lambda msg, * \
            args, **kwargs: original_error(PHISanitizer.sanitize_string(msg), *args, **kwargs)
        logger.critical = lambda msg, * \
            args, **kwargs: original_critical(PHISanitizer.sanitize_string(msg), *args, **kwargs)

        #     return logger # FIXME: return outside function
