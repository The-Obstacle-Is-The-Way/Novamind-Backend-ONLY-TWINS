...
def mock_boto3():
    ...
    with patch("boto3.client") as mock_client, patch("boto3.resource") as mock_resource:
        ...
        mock_client.side_effect = lambda service_name, **kwargs: s3_client if service_name == "s3" else MagicMock()

        # Mock DynamoDB resource
        dynamodb = MagicMock()
        mock_resource.side_effect = lambda service_name, **kwargs: dynamodb if service_name == "dynamodb" else MagicMock()

        # Mock Comprehend Medical client
        comprehend_medical = MagicMock()
        mock_client.side_effect = lambda service_name, **kwargs: comprehend_medical if service_name == "comprehendmedical" else MagicMock()
...
class TestAWSPATService:
    ...
    def test_initialization_failure(self, aws_config, mock_boto3):
