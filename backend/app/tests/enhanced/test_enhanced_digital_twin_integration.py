...
async def test_process_multimodal_data(
    ...
    behavioral_data = {
        ...
        "sleep": {"hours_per_night": [6.2, 5.8, 6.5, 7.0, 6.3, 6.7, 6.1]},
    }
...
async def test_knowledge_graph_operations(
    ...
    updated_graph = await digital_twin_service.update_knowledge_graph(
        patient_id=patient_id, new_data=new_data, data_source="clinical_update"
    )
    ...
    assert any(
        node.label == "Major Depressive Disorder"
        for node in updated_graph.nodes.values()
    )
    assert any(
        node.label == "Generalized Anxiety Disorder"
        for node in updated_graph.nodes.values()
    )

    # Verify that the new diagnosis node exists
    treatment_resistant_nodes = [
        node
        for node in updated_graph.nodes.values()
...
async def test_belief_network_operations(
    ...
    evidence = {"mood": "depressed", "sleep": "insomnia"}

    # Update the belief network
    updated_network = await digital_twin_service.update_belief_network(
        patient_id=patient_id, evidence=evidence, source="clinical_assessment"
    )
...
async def test_event_system(enhanced_services, patient_id):
    ...
    await digital_twin_service.initialize_digital_twin(
        patient_id=patient_id, initial_data={}
    )
