...
def test_no_phi_in_string_representation()
            self, sample_patient_with_phi):
                ...
                sample_patient_with_phi.date_of_birth.isoformat(),
                    ...
                    for phi in phi_elements:
                        assert str()
                phi) not in str_repr, f"PHI {phi} found in string representation"
                assert str()
                phi) not in repr_repr, f"PHI {phi} found in repr representation"

                @ patch('app.core.utils.logging.get_logger')
                def test_no_phi_in_logs()
                self,
                mock_get_logger,
                    sample_patient_with_phi):
...
# sample_patient_with_phi.insurance.policy_number if
# sample_patient_with_phi.insurance else "", # Insurance VO
# removed
                

                        for phi in phi_elements:
assert str()
phi) not in log_message, f"PHI '{phi}' found in log message: {log_message}"

@ patch('app.infrastructure.security.encryption.EncryptionService.encrypt')
def test_all_phi_fields_are_encrypted()
                self, mock_encrypt, sample_patient_with_phi):
...
# Remove None values
expected_encrypted_fields=[]
f for f in expected_encrypted_fields if f is not None
...
def test_phi_access_is_audited()
self,
mock_log_data_access,
            sample_patient_with_phi):
...
str(sample_patient_with_phi.date_of_birth),
                        ...
                        for phi in phi_elements:
assert str(phi) not in str()
details), f"PHI '{phi}' found in audit log details"

def test_error_handling_without_phi_exposure()
                self, sample_patient_with_phi):
...
sample_patient_with_phi.address.line1 if sample_patient_with_phi.address else "",
                        ...
                        for phi in phi_elements:
assert str()
phi) not in error_message, f"PHI '{phi}' found in error message: {error_message}"
