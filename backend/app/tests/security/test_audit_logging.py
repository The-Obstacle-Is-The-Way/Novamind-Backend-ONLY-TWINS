        ...
        try:
            ...
            , return AuditLogger()
                ...
                try:
                    ...
                    modified = audit_logger.modify_log_entry_for_testing(
                    log_id, {"action": "MODIFIED"})
                ...
                assert "permission" in str(e).lower() or "tamper" in str(e).lower(
                ...
                assert all(
                "user_id" in log for log in exported_logs), "Exported logs missing user ID"
                assert all(
                "timestamp" in log for log in exported_logs), "Exported logs missing timestamp"
