...
class TestMockPHIDetection(BaseSecurityTest):
    ...
    def test_detect_phi_basic(self) -> None:
        ...
        self.assert In("phi_instances", result)
        self.assert IsInstance(result["phi_instances"], list)
        self.assert Greater(len(result["phi_instances"]), 0)
        
        # Check individual PHI instance structure
        for instance in result["phi_instances"]:
            self.assert In("type", instance)
            self.assert In("text", instance)
            self.assert In("position", instance)
            self.assert In("confidence", instance)
    ...
    def test_detect_phi_with_levels(self) -> None:
        ...
        for level in ["minimal", "moderate", "aggressive"]:
            ...
            self.assert In("phi_instances", result)
            self.assert In("detection_level", result)
            self.assert Equal(result["detection_level"], level)
            level_counts[level] = len(result["phi_instances"])
        
        # Aggressive should find more PHI than minimal
        self.assert LessEqual(level_counts["minimal"], level_counts["aggressive"])
    ...
    def test_redact_phi_basic(self) -> None:
        ...
        # Check structure of result
        self.assert In("redacted_text", result)
        self.assert In("phi_instances", result)
        self.assert IsInstance(result["phi_instances"], list)
        
        # Verify that common PHI is redacted
        redacted_text = result["redacted_text"]
        self.assert NotIn("123-45-6789", redacted_text)
        self.assert NotIn("john.smith@example.com", redacted_text)
        
        # Verify default redaction marker is present
        self.assert In("[REDACTED]", redacted_text)
    ...
    def test_redact_phi_custom_replacement(self) -> None:
        ...
        self.assert In(marker, result["redacted_text"])
        
        # Verify original PHI is gone
        self.assert NotIn("123-45-6789", result["redacted_text"])
    ...
    def test_redact_phi_levels(self) -> None:
        ...
        self.assert LessEqual(minimal_redactions, aggressive_redactions)
    ...
    def test_redact_phi_edge_cases(self) -> None:
        ...
        self.assert NotIn(double_redacted, result["redacted_text"])
