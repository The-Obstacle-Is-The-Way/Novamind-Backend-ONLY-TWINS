...
class TestPHIAuditLogic(BaseSecurityTest):
    ...
    def test_audit_result_allowed_status(self):
        ...
        # Add PHI
        result.add_phi_instance(phi_type="SSN", value="123-45-6789", line_num=5, line_content="ssn = '123-45-6789'", context="Function that tests SSN detection")
    ...
    def test_report_counts_for_clean_app_files(self, temp_dir):
        ...
        assert report.files_with_phi == report.files_with_allowed_phi, "All PHI files should be counted as allowed"

        # The audit should have passed
        assert report.to_dict()["files_with_phi"] <= report.to_dict()["files_with_allowed_phi"], "Report should indicate that audit passed"
    ...
    def test_run_audit_with_clean_app_directory(self, mock_logger, temp_dir):
        ...
        assert result.files_with_phi == result.files_with_allowed_phi, "All PHI should be allowed in clean_app directory"

        # Verify audit passed
        assert result.files_with_phi <= result.files_with_allowed_phi, "Audit should pass for clean_app directory"
