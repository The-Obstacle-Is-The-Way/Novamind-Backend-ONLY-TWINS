...
#     return "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ0ZXN0X3VzZXIiLCJyb2xlIjoicGF0aWVudCIsImV4cCI6OTk5OTk5OTk5OX0.signature"@pytest.fixture
    def mock_user():
...
class TestAuthentication:
    ...
    with patch('app.presentation.api.auth.AuthHandler.decode_token', side_effect=HTTPException(status_code=401, detail="Token expired")):
    ...
    with patch('app.presentation.api.auth.AuthHandler.decode_token', side_effect=HTTPException(status_code=401, detail="Invalid token")):
...
class TestAuthorization:
            ...
            with patch('app.presentation.api.auth.get_current_user', return_value=mock_user), \
                 patch('app.presentation.api.endpoints.patients.verify_patient_access', side_effect=HTTPException(status_code=403, detail="Access denied")):
            ...
            with patch('app.presentation.api.auth.get_current_user', return_value=mock_user), \
                 patch('app.presentation.api.endpoints.admin.verify_admin_access', side_effect=HTTPException(status_code=403, detail="Admin access required")):
...
# Ensure wildcard is not used for HIPAA compliance
            assert response.headers["Access-Control-Allow-Origin"] != "*"class TestErrorHandling:
