...
from fastapi.testclient import TestClient # Keep for potential sync tests if any remain
...
class TestAuthentication(BaseSecurityTest):
    ...
    @pytest.fixture(scope="function") # CHANGED: Scope changed to function to match dependency
    ...
    async def test_missing_token(self, client: AsyncClient, patient_user_id: str):
        """Test that requests without tokens are rejected by FastAPI/dependencies."""
        response = await client.get(f"/api/v1/patients/{patient_user_id}") # Use await with AsyncClient
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        # Check WWW-Authenticate header if detailed checks are needed
        # assert "Bearer" in response.headers.get("WWW-Authenticate", "")

    @pytest.mark.asyncio # Added asyncio marker
    async def test_invalid_token_format(self, client: AsyncClient, patient_user_id: str):
        """Test that structurally invalid tokens are rejected."""
        headers = {"Authorization": "Bearer invalid.token.format"}
        response = await client.get(f"/api/v1/patients/{patient_user_id}", headers=headers) # Use await with AsyncClient
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    @pytest.mark.asyncio # Added asyncio marker
    async def test_expired_token(self, client: AsyncClient, generate_token: Callable[[Dict[str, Any]], Coroutine[Any, Any, str]], mock_patient_payload: Dict[str, Any]):
        ...
        response = await client.get(f"/api/v1/patients/{user_id}", headers=headers)
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        # Check detail if specific error message is expected for expired tokens
        # assert "expired" in response.json().get("detail", "").lower()

    @pytest.mark.asyncio # Added asyncio marker
    async def test_tampered_token(self, client: AsyncClient, patient_user_id: str):
    ...
    @pytest.mark.asyncio # Added asyncio marker
    async def test_valid_token_access(self, client: AsyncClient, patient_token_headers: Dict[str, str], mock_patient_payload: Dict[str, Any]):
        ...
        response = await client.get(f"/api/v1/patients/{user_id}", headers=patient_token_headers)
...
class TestAuthorization(BaseSecurityTest):
    ...
    @pytest.mark.asyncio # Added asyncio marker
    async def test_patient_accessing_own_data(self, client: AsyncClient, generate_token: Callable[[Dict[str, Any]], Coroutine[Any, Any, str]], mock_patient_payload: Dict[str, Any]):
        ...
        response = await client.get(f"/api/v1/patients/{user_id}", headers=headers) # Use await with AsyncClient
    ...
    @pytest.mark.asyncio # Added asyncio marker (assuming client call might be async under the hood)
    async def test_patient_accessing_other_patient_data(self, client: AsyncClient, patient_token_headers: Dict[str, str], mock_patient_payload: Dict[str, Any]):
        ...
        response = await client.get(f"/api/v1/patients/{other_patient_id}", headers=patient_token_headers) # Use await with AsyncClient
        
        # Expect 403 Forbidden (or potentially 404 if combined)
        assert response.status_code == status.HTTP_403_FORBIDDEN

    @pytest.mark.asyncio # Added asyncio marker (assuming client call might be async under the hood)
    async def test_provider_accessing_patient_data(self, client: AsyncClient, provider_token_headers: Dict[str, str], mock_patient_payload: Dict[str, Any]):
        ...
        response = await client.get(f"/api/v1/patients/{patient_id}", headers=provider_token_headers) # Use await with AsyncClient
    ...
    @pytest.mark.asyncio # Added asyncio marker (assuming client call might be async under the hood)
    async def test_role_specific_endpoint_access(self, client: AsyncClient, patient_token_headers: Dict[str, str], provider_token_headers: Dict[str, str]):
        ...
        response_patient = await client.get(protected_endpoint, headers=patient_token_headers) # Use await with AsyncClient
        ...
        response_provider = await client.get(protected_endpoint, headers=provider_token_headers) # Use await with AsyncClient
...
@pytest.mark.db_required() # Added marker based on likely need
class TestInputValidation(BaseSecurityTest):
    """Test input validation using Pydantic models and FastAPI."""

    @pytest.mark.asyncio # Added asyncio marker (assuming client call might be async under the hood)
    async def test_invalid_input_format_rejected(self, client: AsyncClient, provider_token_headers: Dict[str, str]):
        ...
        response = await client.post("/api/v1/patients", headers=provider_token_headers, json=invalid_payload)
        assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY

    @pytest.mark.asyncio # Added asyncio marker (assuming client call might be async under the hood)
    async def test_input_sanitization_handling(self, client: AsyncClient, provider_token_headers: Dict[str, str]):
        ...
        response = await client.post("/api/v1/patients", headers=provider_token_headers, json=malicious_input)
    ...
    @pytest.mark.asyncio # Added asyncio marker (assuming client call might be async under the hood)
    async def test_input_length_limits_enforced(self, client: AsyncClient, provider_token_headers: Dict[str, str]):
        ...
        response = await client.post("/api/v1/patients", headers=provider_token_headers, json=long_input_payload)
...
class TestSecureHeaders(BaseSecurityTest):
    ...
    async def test_required_security_headers_present(self, client: AsyncClient):
        ...
        response = await client.get("/api/v1/health") # Use await with AsyncClient # Use a simple, likely unauthenticated endpoint
...
class TestErrorHandling(BaseSecurityTest):
    ...
    @pytest.mark.asyncio # Added asyncio marker
    async def test_not_found_error_generic(self, client: AsyncClient, provider_token_headers: Dict[str, str]):
        """Test that accessing a non-existent route returns 404."""
        response = await client.get("/api/v1/non_existent_route_for_testing", headers=provider_token_headers)
    ...
    @pytest.mark.asyncio # Added asyncio marker
    async def test_internal_server_error_masked(self, mock_get_by_id: MagicMock, client: AsyncClient, provider_token_headers: Dict[str, str]):
        ...
        response = await client.get(f"/api/v1/patients/{patient_id}", headers=provider_token_headers)
        ...
        assert "Simulated unexpected database error" not in response.text.lower()
...
async def test_access_patient_phi_data_unauthorized_patient(
    ...
    response = await client.get(f"/api/v1/patients/{target_patient_id}", headers=patient_token_headers)
...
async def test_access_patient_phi_data_patient_not_found(
    ...
    response = await client.get(f"/api/v1/patients/{non_existent_patient_id}", headers=provider_token_headers)
