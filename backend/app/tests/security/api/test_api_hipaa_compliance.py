...
class TestAPIHIPAACompliance:
    ...
    def test_patient_data_isolation(self, client, patient_token):
        ...
        own_response = client.get("/api/v1/patients/P12345", headers={"Authorization": patient_token})
        assert own_response.status_code == 200
        
        # Someone else's data - should be forbidden
        other_response = client.get("/api/v1/patients/P67890", headers={"Authorization": patient_token})
        assert other_response.status_code == 403
        
    def test_phi_sanitization_in_response(self, client, admin_token):
        """Test that PHI is properly sanitized in API responses."""
        response = client.get("/api/v1/patients/P12345", headers={"Authorization": admin_token})
    ...
    def test_phi_not_in_query_params(self, client, admin_token):
        ...
        client.get("/api/v1/patients", headers={"Authorization": admin_token}, params={"name": "John", "status": "active"})
    ...
    def test_phi_not_in_request_bodies(self, client, admin_token):
        ...
        response = client.post("/api/v1/patients", headers={"Authorization": admin_token}, json={
            "first_name": "Jane",
            "last_name": "Smith",
    ...
    def test_proper_authentication_and_authorization(self, client, admin_token, doctor_token, patient_token):
        ...
        admin_response = client.get("/api/v1/patients/P67890", headers={"Authorization": admin_token})
        assert admin_response.status_code == 200
        
        # Doctor can access any patient
        doctor_response = client.get("/api/v1/patients/P67890", headers={"Authorization": doctor_token})
        assert doctor_response.status_code == 200
        
        # Patient can only access their own data
        patient_response = client.get("/api/v1/patients/P67890", headers={"Authorization": patient_token})
    ...
    def test_sensitive_operations_audit_log(self, client, admin_token):
        ...
        client.post("/api/v1/patients", headers={"Authorization": admin_token}, json={"first_name": "Test", "last_name": "Patient"})
