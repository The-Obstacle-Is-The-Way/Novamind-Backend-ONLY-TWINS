    ...
    @pytest.mark.db_required()class Database:
        ...
        def __init__(
            ...
            if self.user_context["role"] not in ["admin", "doctor", "nurse"]:
                ...
                # Apply PHI filtering based on role
                filtered_patients = [
                    self._apply_phi_filters(p) for p in patients]
            ...
            if self.user_context["role"] not in ["admin", "doctor"]:
                ...
                patient.id = "P" + \
                    str(hash(patient.first_name + patient.last_name))[:5]
            ...
            if not self._can_access_patient(patient.id):
                ...
                raise PermissionError(
                    f"Unauthorized to update patient {
                        patient.id}")
            ...
            if self.user_context["role"] != "admin":
                ...
                raise PermissionError(
                    f"Unauthorized to delete patient {patient_id}")
...
def guest_context(self):
        ...
        assert patient_repo.get_by_id(
            "P67890") is None      # Someone else's record
        ...
        assert patient1_repo.get_by_id(
            "P12345") is not None  # Their own record
        assert patient1_repo.get_by_id(
            "P67890") is None      # Someone else's record

        assert patient2_repo.get_by_id(
            "P67890") is not None  # Their own record
        assert patient2_repo.get_by_id(
            "P12345") is None      # Someone else's record
