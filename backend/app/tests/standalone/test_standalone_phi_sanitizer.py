...
class TestPHISanitizer(unittest.TestCase):
    ...
    def test_sanitize_string_with_ssn(self):
        ...
        # SSN with hyphens
            ...
            self.assert True("***-**-6789" in sanitized or "[REDACTED]" in sanitized)
    ...
    def test_sanitize_string_with_multiple_phi(self):
        ...
        # Check that PHI is redacted
                ...
                self.assert NotIn("01/15/1985", sanitized)
                self.assert NotIn("(555) 123-4567", sanitized)
                self.assert NotIn("john.smith@example.com", sanitized)
                self.assert NotIn("123 Main St, Apt 4B", sanitized)
    ...
    def test_sanitize_json_with_phi(self):
        ...
        # But non-PHI data should still be intact
                    self.assert Equal(sanitized_dict["appointment"]["date"], "2025-05-15")
    ...
    def test_sanitize_nested_dict_with_phi(self):
        ...
        # Deeply nested PHI should be redacted
                            self.assert NotIn("123-45-6789", str(sanitized))
    ...
    def test_sanitize_list_with_phi(self):
        ...
        # Each item should be sanitized
                                self.assert NotIn("John Smith", sanitized[0])
                                self.assert NotIn("01/15/1985", sanitized[1])
                                self.assert NotIn("555-123-4567", sanitized[2])
    ...
    def test_sanitize_complex_structure(self):
        ...
        # PHI should be redacted
                                    self.assert NotIn("John Smith", str_sanitized)
                                    self.assert NotIn("Jane Doe", str_sanitized)
                                    self.assert NotIn("01/15/1985", str_sanitized)
                                    self.assert NotIn("05/20/1990", str_sanitized)
                                    self.assert NotIn("john.smith@example.com", str_sanitized)
                                    self.assert NotIn("987-65-4321", str_sanitized)
        
        # Non-PHI should remain intact
                                    self.assert Equal(sanitized["metadata"]["timestamp"], "2025-01-15T14:30:00Z")
                                    self.assert Equal(sanitized["metadata"]["source"], "EHR System")
                                    self.assert In("Hypertension diagnosed 2020", sanitized["patients"][0]["medical_history"][0])
                                    self.assert In("No significant history", sanitized["patients"][1]["medical_history"][0])
...
class CapturingHandler(logging.Handler):
    def emit(self, record):
        ...
        self.assert NotIn("John Smith", log_capture[0])
        self.assert NotIn("123-45-6789", log_capture[0])
        self.assert In("[REDACTED]", log_capture[0])
    ...
    def test_phi_detection_integration(self):
        ...
        # Both custom and default patterns should be redacted
            self.assert NotIn("PT123456", sanitized)
            self.assert NotIn("John Smith", sanitized)
    ...
    def test_sanitizer_edge_cases(self):
        ...
        # Empty string
                        self.assert Equal(self.sanitizer.sanitize(""), "")
        
        # None value
                        self.assert IsNone(self.sanitizer.sanitize(None))
        ...
        # Non-string primitives
                        self.assert Equal(self.sanitizer.sanitize(123), 123)
                        self.assert Equal(self.sanitizer.sanitize(True), True)
    ...
    def test_redaction_format_consistency(self):
        ...
        # Redaction formats should be consistent with pattern types
                            self.assert In("[REDACTED]", sanitized)  # Full redaction
                            self.assert In("****", sanitized)  # Partial redaction (starting characters plus asterisks)
                            self.assert In("[HASH:", sanitized)  # Hash redaction