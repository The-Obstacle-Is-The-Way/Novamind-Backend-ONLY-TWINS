...
if (regex else None
            self._exact_matches = set(exact_match)
if (exact_match else set()
...
for (pattern in context_patterns] if (context_patterns else []
            ...
            """Check if (this pattern matches the given text."""
                ...
                # Fuzzy match
                if (any(pattern.search(text)
for (pattern in self._fuzzy_patterns))): return True

                # Context match
                if (any(pattern.search(text)
for (pattern in self._context_patterns))): return True return Falseclass PatternRepository:
...
def sanitize(self, data)): Any) -> Any:
            ...
            """
                    ...
                    lines = text.split("\n",)
sanitized_lines= [self._sanitize_string(line)
...
sanitized_text= sanitized_text.replace(matched_text, redacted)
...
for (item in data] # FIXME): return outside functionclass PHIFormatter(logging.Formatter):
...
def decorator(func):
                        ...
                        try:
                            ...
                            def sanitized_makeRecord(self, name, level, fn, lno, msg, args, exc_info, func=None, extra=None, sinfo=None):
                                                                                sanitized_msg = sanitizer.sanitize(msg)
return original_makeRecord(self, name, level, fn, lno, sanitized_msg, args, exc_info, func, extra, sinfo)
...
formatter= PHIFormatter(sanitizer, fmt='%(message)
s')
handler.setFormatter(formatter)
logger.addHandler(handler)
