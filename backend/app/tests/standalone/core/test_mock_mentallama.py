...
class TestMockMentaLLaMA(BaseUnitTest):
    ...
    service = MockMentaLLaMA()
    service.initialize({"model": "mock-psychiatric-gpt-4"})
    self.assertTrue(service.is_healthy())
        
    # Test with invalid model
    service = MockMentaLLaMA()
        with self.assertRaises(ModelNotFoundError):
            service.initialize({"model": "nonexistent-model"})
        
    # Test with invalid configuration
    service = MockMentaLLaMA()
        with self.assertRaises(InvalidConfigurationError):
            service.initialize({"invalid_param": True})
    ...
    # Test with None
        with self.assertRaises(InvalidRequestError):
            self.service.analyze_text(None)
    ...
    {"role": "therapist", "content": "That sounds difficult. Have you had these feelings before?"},
