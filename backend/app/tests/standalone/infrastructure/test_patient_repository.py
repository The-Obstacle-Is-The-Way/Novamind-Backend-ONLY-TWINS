...
class TestPatientRepository:
    ...
    async def test_create_patient(self):
        ...
        # Configure mock to track commit operation
        self.mock_db._committed_objects = []

        # Create patient
        result = await self.repository.create(self.patient_1)

        # Verify patient was committed to the database
    ...
    async def test_get_patient_by_id_not_found(self):
        ...
        # Get non-existent patient by ID
        result = await self.repository.get_by_id(uuid4())

        # Verify query was executed and empty result returned
        assert self.mock_db._last_executed_query is not None
