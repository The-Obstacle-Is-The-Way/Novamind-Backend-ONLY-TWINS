"""
Standalone mock implementation of the PAT (Patient Actigraphy Tracking) service.
...
class MockPATService:
    ...
    def analyze_actigraphy(
        ...
        if not device_info or "device_type" not in device_info or "manufacturer" not in device_info:
            raise ValidationError("Device info must include device_type and manufacturer")
    ...
    def integrate_with_digital_twin(
        ...
        if analysis["patient_id"] != patient_id:
            raise ValidationError(f"Analysis {analysis_id} does not belong to patient {patient_id}")
