...
class TestMLExceptions(unittest.TestCase):
    ...
    def test_base_error(self):
        ...
        # Check message
        self.assert Equal(str(error), "Test error message")
        self.assert Equal(error.message, "Test error message")
        
        # Check details
        self.assert Equal(error.details, {})
        
        # Add a detail
        error.add_detail("test_key", "test_value")
        
        # Check the detail was added
        self.assert Equal(error.get_detail("test_key"), "test_value")
        
        # Check default value for non-existent detail
        self.assert IsNone(error.get_detail("non_existent"))
        self.assert Equal(error.get_detail("non_existent", "default"), "default")
        
        # Check to_dict
        error_dict = error.to_dict()
        self.assert Equal(error_dict["error_type"], "MLBaseError")
        self.assert Equal(error_dict["message"], "Test error message")
        self.assert Equal(error_dict["details"], {"test_key": "test_value"})
    ...
    def test_inference_error(self):
        ...
        # Check message
        self.assert Equal(error.message, "Failed to run inference")
        
        # Check model name
        self.assert Equal(error.model_name, "test_model")
        
        # Check input data
        self.assert Equal(error.input_data, [1, 2, 3, 4, 5])
        
        # Check details contain model name and input info
        self.assert Equal(error.get_detail("model_name"), "test_model")
        self.assert Equal(error.get_detail("input_length"), 5)
        ...
        self.assert Equal(error.get_detail("input_keys"), ["x", "y"])
    ...
    def test_validation_error(self):
        ...
        # Check message
        self.assert Equal(error.message, "Input validation failed")
        ...
        self.assert Equal(len(error.validation_errors), 2)
        self.assert Equal(error.validation_errors[0]["field"], "input_shape")
        self.assert Equal(error.validation_errors[0]["error"], "Invalid shape")
        self.assert Equal(error.validation_errors[0]["expected"], "(batch_size, 10)")
        self.assert Equal(error.validation_errors[0]["actual"], "(batch_size, 5)")
        
        # Check details
        self.assert Equal(len(error.get_detail("validation_errors")), 2)
    ...
    def test_model_not_found_error(self):
        ...
        # Check message
        self.assert Equal(error.message, "Model 'non_existent_model' not found")
        
        # Check model name
        self.assert Equal(error.model_name, "non_existent_model")
        
        # Check available models
        self.assert Equal(error.available_models, ["model1", "model2", "model3"])
        
        # Check details
        self.assert Equal(error.get_detail("model_name"), "non_existent_model")
        self.assert Equal(error.get_detail("available_models"), ["model1", "model2", "model3"])
    ...
    def test_service_unavailable_error(self):
        ...
        # Check message
        self.assert Equal(error.message, "ML service 'test_service' is unavailable: Maintenance in progress")
        
        # Check service info
        self.assert Equal(error.service_name, "test_service")
        self.assert Equal(error.reason, "Maintenance in progress")
        self.assert Equal(error.retry_after, 3600)
        
        # Check details
        self.assert Equal(error.get_detail("service_name"), "test_service")
        self.assert Equal(error.get_detail("reason"), "Maintenance in progress")
        self.assert Equal(error.get_detail("retry_after"), 3600)
    ...
    def test_rate_limit_error(self):
        ...
        self.assert Equal(error.message, "ML service 'test_service' is unavailable: Rate limit of 100 requests exceeded")
        
        # Check rate limit info
        self.assert Equal(error.service_name, "test_service")
        self.assert Equal(error.reason, "Rate limit of 100 requests exceeded")
        self.assert Equal(error.limit, 100)
        self.assert Equal(error.retry_after, 60)
        
        # Check details
        self.assert Equal(error.get_detail("service_name"), "test_service")
        self.assert Equal(error.get_detail("reason"), "Rate limit of 100 requests exceeded")
        self.assert Equal(error.get_detail("limit"), 100)
        self.assert Equal(error.get_detail("retry_after"), 60)
