...
def test_unauthenticated_access(test_client: TestClient) -> None:
    ...
    response = test_client.get("/api/v1/actigraphy/model-info")
...
def test_authorized_access(test_client: TestClient, patient_token: str) -> None:
    ...
    response = test_client.get("/api/v1/actigraphy/model-info", headers={"Authorization": f"Bearer {patient_token}"})

    # Should return 200 OK
    assert response.status_code == 200


def test_input_validation(test_client: TestClient, patient_token: str) -> None:
    ...
    response = test_client.post("/api/v1/actigraphy/analyze", json=invalid_request, headers={"Authorization": f"Bearer {patient_token}"})
...
def test_phi_data_sanitization(test_client: TestClient, provider_token: str, sample_readings: List[Dict[str, Any]], sample_device_info: Dict[str, Any]) -> None:
    ...
    response = test_client.post("/api/v1/actigraphy/analyze", json=phi_request, headers={"Authorization": f"Bearer {provider_token}"})
    ...
    response = test_client.get(f"/api/v1/actigraphy/analysis/{analysis_id}", headers={"Authorization": f"Bearer {provider_token}"})
...
def test_role_based_access_control(test_client: TestClient, patient_token: str, provider_token: str, admin_token: str, sample_readings: List[Dict[str, Any]], sample_device_info: Dict[str, Any]) -> None:
    ...
    provider_response = test_client.post("/api/v1/actigraphy/analyze", json=provider_request, headers={"Authorization": f"Bearer {provider_token}"})

    assert provider_response.status_code == 200
    analysis_id = provider_response.json()["analysis_id"]

    # Patient can view their own analysis
    patient_view_response = test_client.get(f"/api/v1/actigraphy/analysis/{analysis_id}", headers={"Authorization": f"Bearer {patient_token}"})

    assert patient_view_response.status_code == 200

    # Admin can view any analysis
    admin_view_response = test_client.get(f"/api/v1/actigraphy/analysis/{analysis_id}", headers={"Authorization": f"Bearer {admin_token}"})

    assert admin_view_response.status_code == 200

    # Patient can't access certain admin features
    patient_admin_response = test_client.get("/api/v1/actigraphy/admin/usage-statistics", headers={"Authorization": f"Bearer {patient_token}"})
...
def test_hipaa_audit_logging(test_client: TestClient, provider_token: str, sample_readings: List[Dict[str, Any]], sample_device_info: Dict[str, Any]) -> None:
    ...
    try:
        ...
        # Get the analysis
        test_client.get(f"/api/v1/actigraphy/analysis/{analysis_id}", headers={"Authorization": f"Bearer {provider_token}"})
...
def test_secure_data_transmission(test_client: TestClient, provider_token: str, sample_readings: List[Dict[str, Any]], sample_device_info: Dict[str, Any]) -> None:
    ...
    # Make a request
    capture_client.get(f"/api/v1/actigraphy/analysis/{analysis_id}", headers={"Authorization": f"Bearer {provider_token}"})
...
def test_api_response_structure(test_client: TestClient, provider_token: str, sample_readings: List[Dict[str, Any]], sample_device_info: Dict[str, Any]) -> None:
    ...
    response = test_client.post("/api/v1/actigraphy/analyze", json=request_data, headers={"Authorization": f"Bearer {provider_token}"})
