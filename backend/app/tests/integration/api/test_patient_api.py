...
def test_patient(db_session):
    ...
    @pytest.mark.integration()class TestPatientAPI:
...
saved_patient = db_session.query(PatientModel).filter_by(
    id=patient_data["id"]).first()
 ...
 def test_delete_patient_success(self, api_client, db_session):
       ...
       patient_to_delete = PatientModel(,
        ...
        response = api_client.delete(
            f"/api/v1/patients/{patient_to_delete.id}")

        # Assert
        assert response.status_code == 204

        # Verify the patient was actually deleted from the database
        deleted_patient = db_session.query(PatientModel).filter_by(
            id=patient_to_delete.id).first()
