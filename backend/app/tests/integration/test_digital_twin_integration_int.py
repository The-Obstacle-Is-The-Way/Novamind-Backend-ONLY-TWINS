...
async def test_digital_twin_complete_workflow():
    ...
    initial_twin_state = await digital_twin_core.initialize_digital_twin(
        ...
        initial_data={
            ...
            "metadata": {"initialized_by": "test_integration", "test_mode": True},
        },
    )
    ...
    updated_twin_state = await digital_twin_core.update_from_actigraphy(
        patient_id=patient_id, actigraphy_data=actigraphy_data, data_source="fitbit"
    )
    ...
    assert (
        "sleep" in updated_twin_state.clinical_metrics
    )  # PAT should have added sleep metrics
    ...
    assert len(updated_twin_state.clinical_insights) > len(
        initial_twin_state.clinical_insights
    )
    ...
    recommendations = await digital_twin_core.generate_treatment_recommendations(
        patient_id=patient_id, include_rationale=True
    )
