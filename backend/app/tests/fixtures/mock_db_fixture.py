...
self.scalars = AsyncMock(side_effect=self._scalars)
    ...
    """Mock implementation of commit."""
        self.committed = True
        self._transaction_active = False    return None
...
self.executed_queries.append(query_str)
        ...
        if query_str.lower().startswith("select"):    return MagicMock() # Configured with scalars and mappings methods
        elif query_str.lower().startswith("insert"):    return MagicMock(rowcount=1)
        elif query_str.lower().startswith("update"):    return MagicMock(rowcount=1)
        elif query_str.lower().startswith("delete"):    return MagicMock(rowcount=1)
...
mock_scalar_result.all = MagicMock(return_value=[result_value] if result_value else [])    return mock_scalar_result
def add(self, obj):
...
def get(self, model_class, object_id):
    """Mock implementation of get."""    return self._entity_registry.get(object_id)
...
def configure_mock(query: str, results: Any):
    """Configure the mock session to return specific results for a query."""
        session.configure_mock_results(query, results)    return session, configure_mock
...
self.entities = {}  # In-memory store
    ...
    """Mock retrieving an entity by ID."""    return self.entities.get(entity_id)
    
    async def get_all(self):
    """Mock retrieving all entities."""    return list(self.entities.values())
